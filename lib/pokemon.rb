class Pokemon
  ...
  def self.find (id_num, db_connect)
    ...
    pokemon_info = db_connect.execute("SELECT * FROM pokemon WHERE id=?", id_num).flatten
    Pokemon.new(id: pokemon_info[0], name: pokemon_info[1], type: pokemon_info[2], db: db_connect)
